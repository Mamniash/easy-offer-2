...
export default function TrainerSession() {
  ...
  const fetchTotal = useCallback(async () => {
    ...
    const response = await fetch(
      `/api/tracks/${direction}/questions?page=1&perPage=1`,
      { cache: "no-store" }
    );
    ...
    const firstQuestion = payload.questions?.[0] ?? null;

    setTotalQuestions(total);

    return { total, firstQuestion };
  }, [direction]);
  ...
  const fetchRandomQuestion = useCallback(async () => {
    ...
    try {
      ...
      let firstQuestion: TrainerQuestion | null = null;

      if (total == null) {
        const totalPayload = await fetchTotal();
        if (!totalPayload) {
          return;
        }
        total = totalPayload.total;
        firstQuestion = totalPayload.firstQuestion;
      }
      ...
      const response = await fetch(
        `/api/tracks/${direction}/questions?page=${randomPage}&perPage=1`,
        { cache: "no-store" }
      );
      ...
      const payload = (await response.json()) as TotalResponse;
      const nextQuestion = payload.questions?.[0] ?? firstQuestion;

      if (!nextQuestion) {
        if (requestIdRef.current === requestId) {
          setQuestion(null);
          setError("Вопросы не найдены для выбранного направления.");
        }
      } else {
        if (requestIdRef.current === requestId) {
  ...
  }, [direction, fetchTotal, totalQuestions]);
