...
import type { TaskDirectionGroup } from "@/lib/tasks";

type TasksDirectoryProps = {
  directionGroups: TaskDirectionGroup[];
};

export default function TasksDirectory({ directionGroups }: TasksDirectoryProps) {
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-gray-200 bg-white/80 p-6 shadow-sm md:flex-row md:items-center md:justify-between">
        ...
        <label className="relative w-full md:w-80">
          ...
          <Input
            ...
            placeholder="Найти направление задач"
      ...
      {filteredGroups.map((group) => (
        <div key={group.title} className="space-y-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm uppercase tracking-[0.18em] text-gray-500">{group.title}</span>
          </div>
